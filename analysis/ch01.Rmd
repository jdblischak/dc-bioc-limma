...
```{r}
results_lm <- data.frame(effect_size = stats$lm_beta,
                         std_dev = stats$sd,
                         de = stats$lm_p < 0.05)
```
...
ggplot(results_lm, aes(x = std_dev, y = effect_size, color = de)) +
  geom_point()
```
...
```{r}
results_limma <- data.frame(effect_size = stats$logFC,
                            std_dev = stats$sd,
                            de = stats$adj.P.Val < 0.05)
```
